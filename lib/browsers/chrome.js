	...
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			Network.requestWillBeSent((params) => {
				...
				if (tab.prerender.logRequests || this.options.logRequests) logSentReq(params, tab);
			...
			Network.responseReceived((params) => {
				...
				if(tab.prerender.requests[params.requestId]) {
					...
					if (tab.prerender.logRequests || this.options.logRequests) logReturnedReq(params, tab);
			...
			if (this.options.logBrowserConsole) Runtime.consoleAPICalled(parseAndLogBrowserConsoleLogs);

			Log.entryAdded((params) => {
				if (tab.prerender.logRequests || this.options.logRequests) {
					util.log('log entries', params.entry)
				};
			});
...
function logSentReq(params, tab) {
	const { url } = params.request
	const { numRequestsInFlight } = tab.prerender
	const coreMessage = `sent     ${url}`.padEnd(90, ' ')
	util.log(grey(`${coreMessage} (in flight: ${numRequestsInFlight})`));
}

function logReturnedReq(params, tab) {
	const url = tab.prerender.requests[params.requestId]
	const { numRequestsInFlight } = tab.prerender
	const coreMessage = `received ${url}`.padEnd(90, ' ')
	util.log(grey(`${coreMessage} (in flight: ${numRequestsInFlight})`));
}
...
const logLevel = {
	...
	trace: chalk.grey('trace   ')
};

function parseAndLogBrowserConsoleLogs(params) {
	const { type, args, stackTrace } = params
	const logHeader = logLevel[type]
	if (type === 'trace') {
		const traceText = stackTrace.callFrames.map(formatCallFrame).join('\n         ')
		util.log(logHeader, traceText)
	} else {
		const logArgs = [ logHeader ]
		logArgs.push(...args.map(formatConsoleArg))
		logArgs.push(grey(formatCallFrame(stackTrace.callFrames[0])))
		util.log(...logArgs);
	}
}

function formatConsoleArg({ type, value, preview }) {
	if (type === 'object') {
		const obj = {}
		preview.properties.forEach(({ name, type, value }) => {
			if (type === 'undefined') value = undefined
			if (type === 'number') value = parseFloat(value)
			// Sadly, we don't have access to nested objects
			if (type === 'object') {
				if (value === 'null') value = null
				else if (value.startsWith('Array')) value = `[${value}]`
				else value = `[${value}]`
			}
			obj[name] = value
		})
...
function formatCallFrame(frame) {
	...
	let text = `${url}:${lineNumber}:${columnNumber}`
	if (functionName) text += ` (${functionName})`
	return text
}
