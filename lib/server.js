...
const LOG_BROWSER_CONSOLE = process.env.LOG_BROWSER_CONSOLE || false;
...
server.init = function(options) {
	...
	this.options.logBrowserConsole = this.options.logBrowserConsole || LOG_BROWSER_CONSOLE;
...
server.firePluginEvent = function(methodName, req, res) {
	return new Promise((resolve, reject) => {
		...
		newRes.setHeader = function(key, value) {
			if (key.toLowerCase() === 'location') req.prerender.redirection = value
			res.setHeader(key, value);
		}

		newRes.redirect = function(location) {
			console.log('redirection location', location)
			newRes.setHeader('location', location);
			newRes.send(302);
		}
...
server._send = function(req, res) {
	const { omittedHeaders = [] } = this.options
	...
	if (req.prerender.renderType == 'html') {
		Object.keys(req.prerender.headers || {}).forEach(function(header) {
			if (omittedHeaders.includes(header.toLowerCase())) return
			try {
				res.setHeader(header, req.prerender.headers[header]);
			} catch (e) {
				util.log('warning: unable to set header:', header, e.message);
			}
		});
	}
	...
	util.log('got', statusCode, 'in', ms + 'ms', 'for', req.prerender.url);
};