...
server._killPhantomJS = function() {
    // hack around this.phantom being sometimes null
    // cf https://github.com/prerender/prerender/issues/246#issuecomment-159212106
    if (this.phantom && this.phantom.process && this.phantom.process.pid) {
        console.log('normal kill', new Date())
        require('tree-kill')(this.phantom.process.pid, 'SIGTERM');
    } else {
        console.log('old-way kill', new Date())
